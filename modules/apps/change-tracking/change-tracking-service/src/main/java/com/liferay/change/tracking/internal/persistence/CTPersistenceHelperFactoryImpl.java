...
public class CTPersistenceHelperFactoryImpl
	...
	private class CTAdapterServiceTrackerCustomizer
		...
		public CTAdapterBag addingService(
			...
			CTAdapterBag ctAdapterBag = new CTAdapterBag(
	...
	private class CTPersistenceHelperImpl
		...
		public List<T> populate(List<T> baseModels) {
			...
			for (int i = 0; i < baseModels.size(); i++) {
				primaryKeys[i] = ctModelAdapter.getPrimaryKey(
					baseModels.get(i));
			}
		...
		public void remove(T model) {
			...
			ctServiceAdapter.removeModelCTs(
				ctModelAdapter.getPrimaryKey(model));
		}
		...
		public void update(T model) {
			...
			if (model.isNew()) {
				...
				ctModelAdapter.setModelCTCollectionId(
					model, _ctCollection.getCtCollectionId());
			}
			else {
				long primaryKey = ctModelAdapter.getPrimaryKey(model);
				...
				if (oldModel != null) {
					...
					ctModelAdapter.populateModel(model, oldModelCT);
				}
			}
			...
			try {
				_ctManager.registerModelChange(
					...
					ctModelAdapter.getPrimaryKey(model), 0, changeType, false);
			}
